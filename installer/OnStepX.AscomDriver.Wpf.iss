; OnStepX (WPF Hub) — Inno Setup script
; Build: iscc OnStepX.AscomDriver.Wpf.iss
; Prereq: Release build of OnStepX.Driver, OnStepX.Shared, OnStepX.Hub.Wpf
;         under src\*\bin\Release\
;
; Mirror of OnStepX.AscomDriver.iss, but ships OnStepX.Hub.Wpf.exe instead of
; the WinForms hub. Distinct AppId so the WPF installer can sit alongside (or
; replace, by registry overlap) the legacy installer without confusing
; Programs & Features. Both installers register the SAME ASCOM driver CLSID,
; so installing one then the other simply overwrites the driver bits and
; rewrites the InstallPath/WpfInstallPath registry hints.

#define MyAppName "OnStepX ASCOM + Hub (WPF)"
...
#endif
#define HubExe     "OnStepX.Hub.Wpf.exe"
#define DriverDll  "ASCOM.OnStepX.Telescope.dll"
#define SharedDll  "OnStepX.Shared.dll"
; Distinct AppId from the legacy WinForms installer so both can coexist in
; Programs & Features. Same CLSID/ProgID for the COM driver — installing this
; package after the legacy one updates the InprocServer32 keys in place.
#define MyAppAppId "{{B8E5C9D2-7A3E-4F6B-9C2D-8E4F5A6B7C8D}"
...
; with the assembly manifest. build-installer-wpf.cmd derives this from /DVERSION.
...
#define HubSrc     "..\src\OnStepX.Hub.Wpf\bin\Release"
...
OutputDir={#OutRoot}
OutputBaseFilename=OnStepX-Setup-Wpf-{#MyAppVersion}
Compression=lzma2
...
; Hub install-path registry hints — HubLauncher in the driver reads WpfInstallPath
; first and falls back to InstallPath. Write both pointing at the WPF exe so a
; legacy install left behind by the previous installer is overridden cleanly.
Root: HKLM; Subkey: "SOFTWARE\OnStepX\Hub";                               ValueType: string; ValueName: "WpfInstallPath"; ValueData: "{app}\{#HubExe}";                                                              Flags: uninsdeletevalue
...
// Best-effort: kill anything still holding a handle on the install dir during
// upgrade — legacy LocalServer exe, legacy WinForms hub, or a stale WPF hub.
// CloseApplications=force handles most cases; this is belt-and-suspenders.
